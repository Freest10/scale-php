 ...
 *	@copyright	Authors
 */
var baseUrlWindow = window.location.protocol+'//'+window.location.hostname;
...
$.richFilemanagerPlugin = function(element, pluginOptions)
{
	 ...
	 */
	var defaults = {
		baseUrl: baseUrlWindow,	// relative path to the FM plugin folder
        ...
        callbacks: {
			...
			afterSelectItem: function (resourceObject, url) { 
				var pathFile = '/files'+resourceObject.id;
				window.parent.$('.modal').trigger('click');
				window.parent.fileInput.val(pathFile);
			}
		}
	};
	...
	var configure = function() {
		return $.when(loadConfigFile('default'), loadConfigFile('user')).done(function(confd, confu) {
			...
			if(config.api.connectorUrl) {
				...
				var connectorUrl = baseUrlWindow;
				var langConnector = '/admin/logged/api/connectors/';
		console.log(connectorUrl);
				...
				apiConnector = baseUrlWindow + langConnector;
			}
		});
	};
	...
	var localize = function() {
		...
		var lang = window.parent.lang;
		var langPath = fm.settings.baseUrl + '/admin/logged/api/filemanager_language/?lang='+lang;
	...
	var includeTemplates = function() {
		return $.when(loadTemplate('upload_container'), loadTemplate('upload_item')).done(function(uc, ui) {
	...
	var includeAssets = function(callback) {
        ...
        primary.push('/css/administration/file_manager/theme.css');

        if(config.customScrollbar.enabled) {
            primary.push('/js/administration/elfinder/custom-scrollbar-plugin/jquery.mCustomScrollbar.min.css');
            primary.push('/js/administration/elfinder/custom-scrollbar-plugin/jquery.mCustomScrollbar.concat.min.js');
        }
		...
		if(config.viewer.editable.enabled) {
            secondary.push('/js/administration/elfinder/CodeMirror/lib/codemirror.css');
            secondary.push('/js/administration/elfinder/CodeMirror/theme/' + config.viewer.editable.theme + '.css');
            secondary.push('/js/administration/elfinder/CodeMirror/lib/codemirror.js');
            secondary.push('/js/administration/elfinder/CodeMirror/addon/selection/active-line.js');
            secondary.push('/js/administration/elfinder/CodeMirror/addon/display/fullscreen.css');
            secondary.push('/js/administration/elfinder/CodeMirror/addon/display/fullscreen.js');
		}

		if(!config.options.browseOnly) {
			// Loading jquery file upload library
            secondary.push('/js/administration/elfinder/jQuery-File-Upload/js/vendor/jquery.ui.widget.js');
            secondary.push('/js/administration/elfinder/jQuery-File-Upload/js/canvas-to-blob.min.js');
            secondary.push('/js/administration/elfinder/jQuery-File-Upload/js/load-image.all.min.js');
            secondary.push('/js/administration/elfinder/jQuery-File-Upload/js/jquery.iframe-transport.js');
            secondary.push('/js/administration/elfinder/jQuery-File-Upload/js/jquery.fileupload.js');
            secondary.push('/js/administration/elfinder/jQuery-File-Upload/js/jquery.fileupload-process.js');
            secondary.push('/js/administration/elfinder/jQuery-File-Upload/js/jquery.fileupload-image.js');
            secondary.push('/js/administration/elfinder/jQuery-File-Upload/js/jquery.fileupload-validate.js');

			if(config.upload.multiple) {
                secondary.push('/js/administration/elfinder/jQuery-File-Upload/css/dropzone.css');
			}
		}

		if(config.options.charsLatinOnly) {
            secondary.push('/js/administration/elfinder/speakingurl/speakingurl.min.js');
		}
		...
		var PreviewModel = function() {
			...
			this.load = function(resourceObject) {
				...
				if(isOpenDocFile(filename) && config.viewer.opendoc.enabled === true) {
					...
					viewerObject.url = fm.settings.baseUrl + '/js/administration/elfinder/ViewerJS/index.html#' + createPreviewUrl(resourceObject, true);
					viewerObject.options = {
				...
				// zeroClipboard code
				ZeroClipboard.config({swfPath: fm.settings.baseUrl + '/js/administration/elfinder/zeroclipboard/dist/ZeroClipboard.swf'});
	...
	var loadConfigFile = function (type) {
		...
		if(type === 'user') {
			if($.urlParam('config') != 0) {
				url = fm.settings.baseUrl + '/admin/logged/config/' + $.urlParam('config');
			} else {
				url = fm.settings.baseUrl + '/admin/logged/config/filemanager.config.json';
			}
		} else {
			url = fm.settings.baseUrl + '/admin/logged/config/filemanager.config.default.json';
		}
	...
	var loadTemplate = function(id, data) {
		return $.ajax({
			type: 'GET',
			url: fm.settings.baseUrl + '/admin/logged/api/' + id + '/',
			error: handleAjaxError
		});
	};
					...
					.on('fileuploadadd', function(e, data) {
						...
						$.each(data.files, function (index, file) {
							...
							var $template = $(tmpl('tmpl-upload-item', {
								file: file,
								lang: lg,
								imagesPath: fm.settings.baseUrl + '/js/administration/elfinder/jQuery-File-Upload/img'
							}));
	...
	var instantiateCodeMirror = function(extension) {
		...
		if (!config.viewer.editable.codeHighlight) {
			...
			if (extension === 'js') {
                assets.push('/js/administration/elfinder/CodeMirror/mode/javascript/javascript.js');
				currentMode = 'javascript';
			}
			if (extension === 'css') {
                assets.push('/js/administration/elfinder/CodeMirror/mode/css/css.js');
				currentMode = 'css';
			}
			if (extension === 'html') {
                assets.push('/js/administration/elfinder/CodeMirror/mode/xml/xml.js');
			...
			if (extension === 'php') {
                assets.push('/js/administration/elfinder/CodeMirror/mode/htmlmixed/htmlmixed.js');
                assets.push('/js/administration/elfinder/CodeMirror/mode/xml/xml.js');
                assets.push('/js/administration/elfinder/CodeMirror/mode/javascript/javascript.js');
                assets.push('/js/administration/elfinder/CodeMirror/mode/css/css.js');
                assets.push('/js/administration/elfinder/CodeMirror/mode/clike/clike.js');
                assets.push('/js/administration/elfinder/CodeMirror/mode/php/php.js');
				currentMode = 'application/x-httpd-php';
			}
			if (extension === 'sql') {
                assets.push('/js/administration/elfinder/CodeMirror/mode/sql/sql.js');
